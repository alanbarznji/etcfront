...
import DeleteProductModal from './components/DeletePrinter';
import UpdateProductModal from './components/UpdatePrinterModal';
 
import { Search, X } from 'lucide-react';
import { DeletePrinterAction, GetPrinterAction, GetPrinterSearchAction } from '../../../redux/action/PrinterAction';
...
await dispatch(GetPrinterSearchAction(e.target.value))
  }
 
  
    useEffect(()=>{
  dispatch(GetPrinterAction())
 
    },[Loading])
    const product=useSelector(state=>state.printer.printers.data)
...
console.log(state);
 ...
 <DeleteProductModal isOpen={Delete} DeleteHandle={
  async ()=>{
    setLoading(false)
    await dispatch(DeletePrinterAction(ProductId))
    setLoading(true)
    setDelete(false)
